...
# Balanced concurrency: Fast enough to save time, low enough to prevent 1GB RAM memory crashes
MAX_WORKERS = 3 
...
def process_pdf_pages(pdf_bytes):
    ...
    for page_num in range(len(doc)):
        ...
        # 1. Extract pure embedded screenshot files (prevents overlapping borders)
        screenshots_info = []
        ...
        # 2. Render the full page for Gemini at a lower, network-friendly resolution to save memory
...
if 'final_df' in st.session_state and not st.session_state.final_df.empty:
    ...
    # Updated width command to fix Streamlit deprecation warning
